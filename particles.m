...
N_div = ceil(sqrt(p));
x_j = transpose(repmat(1:N_div,[1 N_div]));
y_j = transpose(repelem(1:N_div,N_div));

%delete additional particles created by ceil() function
x_j = x_j(1:end-(length(x_j)-p),:);
y_j = y_j(1:end-(length(y_j)-p),:);
%move all particles slightly, to ensure they are within the box
x_j = L.*(1/(max(x_j)) .* x_j) - 4*radius;
y_j = L.*(1/(max(y_j)) .* y_j) - 4*radius;
...
for circ = 1:sims
    ...
    % save x- and y- initial positions of all particles
    for i = 1:p
        x(i,1) = x_0(i,circ);
        y(i,1) = y_0(i,circ);
    end
